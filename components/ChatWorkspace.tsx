...
const ChatWorkspace: React.FC<ChatWorkspaceProps> = ({ 
  ...
  useEffect(() => {
    if (autoSubmit && initialPrompt && messages.length === 0) handleSend(initialPrompt);
  }, []);
  ...
  const handleSend = useCallback(async (overrideInput?: string) => {
    ...
    if (activeTab === 'studio') {
      ...
      try {
        ...
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: lang === 'si' ? "නිර්මාණය අවසන්." : "Synthesis complete.", imageUrl: url, timestamp: new Date(), type: 'image' }]);
        setLocalInput('');
        onInputChange('');
      } catch (e: any) {
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: `Studio error: ${e.message}`, timestamp: new Date(), type: 'text' }]);
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'assistant', content: res.text, links: res.links, timestamp: new Date(), type: 'text' }]);
          ...
          <div className="flex-1 overflow-y-auto px-2 space-y-1 custom-scrollbar">
            {conversations.map(conv => (
              <div key={conv.id} className="relative group">
                <button onClick={() => handleSwitch(conv.id)} className={`w-full text-left p-4 pr-12 rounded-2xl transition-all ${activeConvId === conv.id ? 'bg-cyan-500/10 text-cyan-600' : 'text-slate-600 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5'}`}>
                ...
                <button onClick={(e) => handleDelete(e, conv.id)} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"><i className="fa-solid fa-trash-can text-xs"></i></button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-950">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
              {(['chat', 'studio', 'vision'] as WorkspaceMode[]).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 md:px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'}`}>
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-12 py-8 overscroll-contain custom-scrollbar">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-30 text-center space-y-6">
              <div className="w-20 h-20 rounded-[40px] border-2 border-dashed border-slate-300 dark:border-white/10 flex items-center justify-center text-3xl">
                <i className={`fa-solid ${activeTab === 'chat' ? 'fa-message' : activeTab === 'studio' ? 'fa-palette' : 'fa-camera'}`}></i>
              </div>
              <p className="text-[10px] font-black uppercase tracking-[0.5em]">{t.howHelp}</p>
            </div>
          ) : (
            <div className="max-w-4xl mx-auto space-y-10 pb-32">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-fade`}>
                  <div className={`max-w-[85%] md:max-w-[70%] p-5 md:p-7 rounded-[32px] ${msg.role === 'user' ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-950 rounded-tr-none shadow-xl' : 'bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-tl-none border border-black/5 dark:border-white/5'}`}>
                    <p className={`text-sm md:text-lg leading-relaxed ${/[^\u0000-\u007F]/.test(msg.content) ? 'sinhala-text' : ''}`}>{msg.content}</p>
                    {msg.imageUrl && <div className="mt-5 rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-black/5"><img src={msg.imageUrl} className="w-full h-auto object-contain" alt="P" /></div>}
                    {msg.links && msg.links.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-black/5 dark:border-white/5 space-y-2">
                        <p className="text-[8px] font-black uppercase text-slate-400 tracking-widest">{t.knowledgeSources}</p>
                        <div className="flex flex-wrap gap-2">
                          {msg.links.map((link, idx) => (
                            <a key={idx} href={link.uri} target="_blank" className="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 hover:underline flex items-center gap-1">
              ...
              {isTyping && <div className="flex gap-2 p-5 bg-slate-100 dark:bg-slate-900 rounded-[24px] w-fit animate-pulse border border-black/5 dark:border-white/5"><div className="w-2 h-2 bg-cyan-600 rounded-full animate-bounce"></div><div className="w-2 h-2 bg-cyan-600 rounded-full animate-bounce [animation-delay:0.2s]"></div><div className="w-2 h-2 bg-cyan-600 rounded-full animate-bounce [animation-delay:0.4s]"></div></div>}
              <div ref={scrollRef} />
            </div>
          )}
        </main>
        ...
        <div className="p-4 md:p-8 shrink-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 z-40">
          <div className="max-w-4xl mx-auto flex items-center gap-2 md:gap-3 bg-slate-100 dark:bg-white/5 p-1.5 md:p-2 rounded-[24px] md:rounded-[36px] shadow-inner focus-within:ring-2 ring-cyan-500/50 transition-all border border-transparent">
            <input ref={inputRef} value={localInput} onChange={e => setLocalInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder={activeTab === 'studio' ? t.placeholderStudio : t.inputPrompt} className="flex-1 bg-transparent border-none focus:ring-0 text-sm md:text-xl py-3 md:py-4 px-4 md:px-6 dark:text-white" />
            <button onClick={() => handleSend()} disabled={isTyping || (!localInput.trim() && activeTab !== 'studio')} className="w-12 h-12 md:w-20 md:h-20 rounded-[18px] md:rounded-[28px] bg-slate-900 dark:bg-white text-white dark:text-slate-950 flex items-center justify-center shadow-2xl hover:scale-105 active:scale-95 transition-all disabled:opacity-50"><i className="fa-solid fa-paper-plane md:text-xl"></i></button>
          </div>
        </div>
      </div>
    </div>
