...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({ onClose, lang, initialMode }) => {
  ...
  const activeColor = mode === 'translator' ? 'amber' : 'amber';
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center p-4 md:p-8 overflow-y-auto custom-scrollbar">
      <button onClick={onClose} className="fixed top-4 right-4 z-[160] w-10 h-10 rounded-xl flex items-center justify-center text-stone-400 hover:text-red-500 hover:bg-black/[0.04] dark:hover:bg-white/[0.05] transition-colors" aria-label="Back to chat"><i className="fa-solid fa-xmark text-lg" /></button>

      <div className="w-full max-w-sm glass-panel rounded-3xl flex flex-col items-center p-6 bg-white dark:bg-slate-900 shadow-xl transition-all duration-300 overflow-hidden relative border border-slate-200/80 dark:border-white/10">
