...
const LiveVisionMode: React.FC<LiveVisionModeProps> = ({ onClose, lang }) => {
  ...
  const startSession = async () => {
    ...
    try {
        ...
        onopen: () => {
          ...
          proc.onaudioprocess = (e) => {
            ...
            for (let i = 0; i < targetLen; i++) {
              ...
              const s = Math.max(-1, Math.min(1, raw[idx] || 0));
              resampled[i] = s * 32767;
            }
      ...
      sessionPromise.then(session => {
        ...
        setConnError("Connection failed. Please try again.");
        stopSession();
      });

    } catch {
      setIsConnecting(false);
      setConnError("Could not access camera/mic. Check permissions and try again.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-black flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Video Viewfinder */}
      {connError && (
        <div className="absolute inset-x-4 top-4 z-50 flex items-center gap-3 px-4 py-3 rounded-xl bg-red-500/90 text-white text-sm font-medium shadow-lg">
          <i className="fa-solid fa-triangle-exclamation shrink-0" />
          <span>{connError}</span>
          <button onClick={() => setConnError(null)} className="ml-auto shrink-0 opacity-70 hover:opacity-100">
